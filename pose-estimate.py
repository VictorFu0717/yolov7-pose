...
@torch.no_grad()
def run(poseweights="yolov7-w6-pose.pt",poseweights2='min_loss_model.pt',source="football1.mp4",device='cpu',view_img=False,
        save_conf=False,line_thickness = 3,hide_labels=False, hide_conf=True, mode=2):
    ...
    if mode == 2:
        model2 = mlp.MLP()
        model2.load_state_dict(torch.load(poseweights2))
        model2.to(device)
    ...
    if (cap.isOpened() == False):   #check if videocapture not opened
        ...
        result_x = eval(config['main']['label_x'])
        result_y = eval(config['main']['label_y'])
        ...
        while(cap.isOpened): #loop until cap opened or video not complete
            ...
            if ret: #if success is true, means frame exist
                ...
                break
        with open(f"label_x.txt", "a", encoding="utf-8") as f:
            f.write(f"{result_x}")
        with open(f"label_y.txt", "a", encoding="utf-8") as f:
            f.write(f"{result_y}")

        config.set('main', 'label_x', str(result_x))
        config.set('main', 'label_y', str(result_y))
        with open('label.ini', 'w') as f:
            config.write(f)
