...
def plot_one_box_kpt(x, im, color=None, label=None, line_thickness=3, kpt_label=False, kpts=None, steps=2, orig_shape=None):
    ...
    if kpt_label:
        point_loc = plot_skeleton_kpts(im, kpts, steps, orig_shape=orig_shape)

    return  point_loc,person_loc
...
def plot_study_txt(path='', x=None):  # from utils.plots import *; plot_study_txt()
    # Plot study.txt generated by train_lstm.py
...
def plot_skeleton_kpts(im, kpts, steps, orig_shape=None):
    ...
    for kid in range(num_kpts):
        ...
        point_loc.append([int(x_coord)/weight, int(y_coord)/height])
        if not (x_coord % 640 == 0 or y_coord % 640 == 0):
            ...
            test = f'({int(x_coord)},{int(y_coord)})'
            x_coord_ = int(x_coord)+5
            y_coord_ = int(y_coord)+10
            loc_ = (x_coord_,y_coord_)
            cv2.putText(im,test,loc_, cv2.FONT_HERSHEY_PLAIN, 1 , (0, 0, 0), 1 , cv2.LINE_AA)
